<?php
/**
 * Uptime Monitor Daemon - Linux Uyumlu
 * Sürekli çalışan monitoring sistemi (Linux hosting için)
 */
...
function runMonitor() {
    ...
    try {
        ...
        writeLog("MULTI-CURL BASLATILIYOR: Toplam " . count($sites) . " site paralel kontrol ediliyor...");
        ...
        writeLog("Paralel kontrol tamamlandi: {$total_elapsed}ms (Ortalama: " . round($total_elapsed / count($sites), 2) . "ms/site)");
                    ...
                    if ($previous_status !== null && $previous_status !== $status) {
                        ...
                        if ($site_data && $site_data['notifications_enabled']) {
                            ...
                            if ($should_notify) {
                                ...
                                writeLog("[MAIL] BILDIRIM GONDERILDI: {$site_name} - {$previous_status} -> {$status} (Site ayarlarına göre)");
                            } else {
                                writeLog("[SKIP] BILDIRIM ATLANDI: {$site_name} - {$previous_status} -> {$status} (Site bildirimleri kapalı)");
                            }
                        } else {
                            writeLog("[SKIP] BILDIRIM ATLANDI: {$site_name} - {$previous_status} -> {$status} (Site bildirimleri devre dışı)");
                        }
                    }
                } else {
                    writeLog("[WARN] HATA: {$site_name} - DB kaydedilemedi!");
                }
            } catch (PDOException $e) {
                writeLog("[ERROR] VERITABANI HATASI: " . $e->getMessage());
                
                // Eğer "MySQL server has gone away" hatası ise, bağlantıyı yenile
                if (strpos($e->getMessage(), 'MySQL server has gone away') !== false || 
                    strpos($e->getMessage(), '2006') !== false) {
                    writeLog("[WARN] MySQL baglantisi koptu, yenileniyor...");
                    reconnectDatabase();
                }
            }
        }
        
        // Sistem ayarlarını güncelle (yeni bağlantı ile)
        try {
            setSystemSetting('last_monitor_run', date('Y-m-d H:i:s'));
            setSystemSetting('last_check_duration_ms', $total_elapsed);
        } catch (Exception $e) {
            writeLog("[ERROR] Sistem ayarlari guncellenemedi: " . $e->getMessage());
            reconnectDatabase();
        }
        
        writeLog("[DONE] Monitoring tamamlandi - Toplam sure: {$total_elapsed}ms");
        
    } catch (Exception $e) {
        writeLog("[ERROR] KRITIK HATA: " . $e->getMessage());
        
        // Kritik hatada da bağlantıyı yenile
        try {
            reconnectDatabase();
        } catch (Exception $reconnect_error) {
            writeLog("[ERROR] Yeniden baglanamadi: " . $reconnect_error->getMessage());
        }
    }
}
...
// Signal handler (Ctrl+C yakalamak için)
declare(ticks = 1);
...
if ($argc < 2) {
    echo "Kullanım: php monitor_daemon_linux.php [start|stop|status|run]\n";
